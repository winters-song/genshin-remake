...
export class Column extends Component {
    ...
    onLoad(): void {
        //先将数据全部加载并处理
        const meshList = this.cloneList(MeshList);
            ...
            this.viewer.load({ url: "Genshin/Login/" + i + ".glb" }).then((v) => {
                ...
                v.traverse((mesh: any) => {
                    if (mesh instanceof Mesh) {
                        ...
                        const instancedMesh = new InstancedMesh(mesh.geometry, mesh.material, columeInstance.indexList.length);
    ...
    update(dt: number): void {
        for (let i of this.columeInstanceMap.values()) {
            for (let j = 0; j < i.indexList.length; j++) {
                if (-i.indexList[j].Location[1] * 0.1 > 2000) {
